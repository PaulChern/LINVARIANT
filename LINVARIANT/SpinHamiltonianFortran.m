...
FortranSpinMCStep[FieldDim_?ListQ] := Module[{Fint, n, numfield, numcomponent, Fout},
  ...
  Fout = {{"Subroutine MCStep(Fields, T, initdamp)"},
    ...
    {Fint[1] <> "Real*8,  Intent(inout) :: Fields(NumField, NGridx, NGridy, NGridz, FieldDim)"},
    ...
    {Fint[1] <> "end do"},
    {Fint[1]},
    {Fint[1] <> "rdum = real(accepted)/real(NumField*NGridx*NGridy*NGridz)"},
    ...
    {Fint[1]},
    {Fint[1] <> "do idum = 1, FieldDim*int((NGridx*NGridy*NGridz)**0.5d0)"},
    {Fint[2] <> "do idelta = 1, NumField"},
    {Fint[3] <> "jdum = mod(idum,3)+1"},
    {Fint[3] <> "diceX = int(ran1(seed)*NGridx) + 1"},
    {Fint[3] <> "diceY = int(ran1(seed)*NGridy) + 1"},
    {Fint[3] <> "diceZ = int(ran1(seed)*NGridz) + 1"},
