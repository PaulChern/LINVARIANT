...
FortranExprFunc[f_?StringQ, FuncType_, expr0_, nn_, level_, OptionsPattern[{"LineLimit" -> 2000, "AllSites" -> False}]] := Module[{Fint, i, n, xyz, term, expr, FoutList, Fout, head, tail, subfunc, test, HeadTail, fvar, euij, tmp}, 
  ...
  Fint[n_] := StringRepeat["  ", n];
  expr = Partition[SortBy[expr0, Quiet@First[DeleteCases[Level[#, {1}], -1]] &], UpTo@OptionValue["LineLimit"]];
  FoutList = Table[tmp = Table[term = ToString[FortranForm[atom]];
                               {If[StringTake[term, 1] != "-", 
                               Fint[level + 1] <> "+" <> StringDelete[term, Whitespace] <> " &", 
                               Fint[level + 1] <> StringDelete[term, Whitespace] <> " &"]}, {atom, expr[[i]]}];
                   SortBy[tmp, ToExpression[First@StringCases[#, RegularExpression["Coeff\\w+\\(\\d*\\)"]]] &], {i, Length@expr}];
