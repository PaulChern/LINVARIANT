...
FortranMain[] := Module[{Fint, Fout},
  ...
  Fout = {{"Program main"},
          ...
          {Fint[1] <> "Real*8            :: e0ij(3,3), de0ijdt(3,3), TK, dene, Etot"},
          ...
          {Fint[1] <> "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"},
          {Fint[1]},
          {Fint[1] <> "INQUIRE(FILE=\"EwaldMat.dat\", EXIST=file_exists)"},
          {Fint[1] <> "if (file_exists) then"},
          {Fint[2] <> "write(*,*) 'EwaldMat.dat exist, checking ...'"},
          {Fint[2] <> "open(ifileno,file='EwaldMat.dat',form='unformatted',status='old')"},
          {Fint[2] <> "read(ifileno) NameEwald, N1, N2, N3"},
          {Fint[2] <> "close(ifileno)"},
          {Fint[2] <> "if(N1.ne.NGridx .or. N2.ne.NGridy .or. N3.ne.NGridz .or. NameEwald .ne. NameSim) then"},
          {Fint[3] <> "write(*,*) 'Dipole file mismatch, current simulation: ', NameSim, NGridx, NGridy, NGridz"},
          {Fint[3] <> "write(*,*) 'Ewald file: ', NameEwald, N1, N2, N3"},
          {Fint[3] <> "write(*,*) 'Now, regenerate Ewald Matrix!!!'"},
          {Fint[3] <> "call EwaldMatrix(alat)"},
          {Fint[2] <> "else"},
          {Fint[3] <> "write(*,*) 'Dipole file matches and will be used.'"},
          {Fint[2] <> "end if"},
          {Fint[2] <> "close(ifileno)"},
          {Fint[1] <> "else"},
          {Fint[2] <> "write(*,*) 'No EwaldMat.dat is found, generating ...'"},
          {Fint[2] <> "call EwaldMatrix(alat)"},
          {Fint[1] <> "end if"},
          {Fint[1]},
          ...
          {Fint[1] <> "!!!!!!!!!!!!!!!!!!!!!"},
          {Fint[1] <> "Call fmkdir(trim(Solver)//\".out\")"},
          ...
          {Fint[2] <> "TK = Thermometer(dFieldsdt, e0ij)"},
          {Fint[2] <> "dFieldsdt = Sqrt(Temp/Tk)*dFieldsdt"},
          {Fint[2] <> "Fields = Sqrt(Temp/Tk)*Fields"},
          {Fint[2] <> "TK = Temp"},
          {Fint[2] <> "gm = 0.0D0"},
          {Fint[2] <> "!!!!!!!!!!!"},
          {Fint[2] <> "! MD LOOP !"},
          ...
          {Fint[3] <> "Call TikTok(Fields, dFieldsdt, e0ij, Temp, TK, gm)"},
          {Fint[3] <> "if(mod(istep,TapeRate).eq.0) then"},
          {Fint[4] <> "tstart = tend"},
          {Fint[4] <> "Call get_walltime(tend)"},
          {Fint[4] <> "write(*,*) trim(Solver)//\" step: \", istep, tend - tstart, \"(second)\", TK*Hartree/k_bolt_ev, gm"},
          {Fint[4] <> "write(*,*) \"Etot: \", GetEtot(Fields, e0ij) + TK*NGridx*NGridy*NGridz*OnSiteDim"},
          {Fint[4] <> "write(*,*) e0ij(1,1), e0ij(2,2), e0ij(3,3)"},
          {Fint[4] <> "write(*,*) e0ij(2,3), e0ij(1,3), e0ij(1,2)"},
          {Fint[4] <> "Call WriteBinary(ifileno, Fields, e0ij)"},
          {Fint[3] <> "end if"},
          {Fint[2] <> "end do"},
          {Fint[2] <> "close(ifileno)"},
          {Fint[2] <> "Call WriteFinal('FinalConfig-'//trim(FileCode)//'.dat', Fields, e0ij)"},
          {Fint[2] <> "Call WriteFinal('FinalVelocity-'//trim(FileCode)//'.dat', dFieldsdt, e0ij)"},
          ...
          {Fint[2] <> "do istep = 1, NumSteps"},
          {Fint[3] <> "Call MCMCStep(istep, seed, Fields, EwaldField, e0ij, Temp, udamp, etadamp, dene)"},
          ...
          {Fint[4] <> "write(*,*) \"Etot: \", GetEtot(Fields, e0ij)"},
          {Fint[4] <> "write(*,*) e0ij(1,1), e0ij(2,2), e0ij(3,3)"},
          {Fint[4] <> "write(*,*) e0ij(2,3), e0ij(1,3), e0ij(1,2)"},
          {Fint[4] <> "Call WriteBinary(ifileno, Fields, e0ij)"},
          {Fint[3] <> "end if"},
          ...
          {Fint[2] <> "Call BinaryToData(TrajectoryFile, 0)"},
          {Fint[2] <> "Call GetObservables(\"Observables\", 1, 0)"},
          {Fint[1] <> "else if(trim(Solver).eq.\"PTMC\") then"},
          {Fint[2] <> "!!!!!!!!!!!!!!!!!!!"},
          {Fint[2] <> "! Initialize PTMC !"},
          {Fint[2] <> "!!!!!!!!!!!!!!!!!!!"},
          ...
          {Fint[4] <> "do istep = 1, CoolingSteps"},
          {Fint[5] <> "Call MCMCStep(istep, seed, Fields, EwaldField, e0ij, &"},
          {Fint[5] <> "TempList(PROCESS_RANK+1)*k_bolt_ev/Hartree, udamp, etadamp, dene)"},
          {Fint[4] <> "end do"},
          {Fint[4] <> "if(ireplica.eq.PROCESS_RANK+1) then"},
          {Fint[5] <> "Call WriteFinal('FinalConfig-'//trim(FileCode)//'.dat', Fields, e0ij)"},
          {Fint[5] <> "Call WriteFinal('FinalVelocity-'//trim(FileCode)//'.dat', dFieldsdt, e0ij)"},
          {Fint[4] <> "end if"},
          {Fint[4] <> "Call PTSwap(TempList, Fields, EwaldField, e0ij, SIZE_CLUSTER, Replicas, PROCESS_RANK+1, ireplica)"},
          ...
          {Fint[2]},
          {Fint[2] <> "do istep = 1, NumSteps"},
          {Fint[3] <> "Call MCMCStep(istep, seed, Fields, EwaldField, e0ij, &"},
          {Fint[3] <> "TempList(PROCESS_RANK+1)*k_bolt_ev/Hartree, udamp, etadamp, dene)"},
          {Fint[3] <> "if((mod(istep,TapeRate).eq.0).and.(istep.gt.ThermoSteps)) then"},
          {Fint[4] <> "if(PROCESS_RANK == 0) then"},
          {Fint[5] <> "tstart = tend"},
          {Fint[5] <> "Call get_walltime(tend)"},
          {Fint[5] <> "write(*,*) trim(Solver)//\" step: \", istep, tend - tstart, \"(second)\""},
          {Fint[4] <> "end if"},
          {Fint[4] <> "Call WriteBinary(ifileno+100*PROCESS_RANK, Fields, e0ij)"},
          {Fint[3] <> "end if"},
          {Fint[3] <> "if((mod(istep,SwapRate).eq.0).and.(istep.lt.ThermoSteps)) then"},
          {Fint[4] <> "Call PTSwap(TempList, Fields, EwaldField, e0ij, SIZE_CLUSTER, Replicas, PROCESS_RANK+1, istep)"},
          {Fint[4] <> "if(PROCESS_RANK == 0) then"},
          {Fint[5] <> "write(11, \"(\"//trim(int2str(SIZE_CLUSTER))//\"E15.6)\") (TempList(i), i=1,SIZE_CLUSTER)"},
          {Fint[5] <> "write(11, \"(\"//trim(int2str(SIZE_CLUSTER))//\"I8)\") (Replicas(i,1), i=1,SIZE_CLUSTER)"},
          {Fint[5] <> "write(11, \"(\"//trim(int2str(SIZE_CLUSTER))//\"I8)\") (Replicas(i,2), i=1,SIZE_CLUSTER)"},
          {Fint[5] <> "write(11, \"(\"//trim(int2str(SIZE_CLUSTER))//\"I8)\") (Replicas(i,3), i=1,SIZE_CLUSTER)"},
          {Fint[5] <> "write(11, \"(\"//trim(int2str(SIZE_CLUSTER))//\"I8)\") (Replicas(i,4), i=1,SIZE_CLUSTER)"},
          {Fint[5] <> "write(11, \"(\"//trim(int2str(SIZE_CLUSTER))//\"I8)\") (Replicas(i,5), i=1,SIZE_CLUSTER)"},
          {Fint[4] <> "end if"},
          {Fint[3] <> "end if"},
...
FortranInputs[] := Module[{Fint, Fout},
  ...
  Fout = {{"Module Inputs"},
          ...
          {Fint[1] <> "!---------------------------------------------------------------------------------"},
          {Fint[1] <> "character(len=20) :: RestartFields    !< restart file"},
          {Fint[1] <> "character(len=20) :: RestartVelocity  !< restart file"},
          ...
          {Fint[5] <> "Temp = Temp*k_bolt_ev/Hartree"},
          {Fint[5] <> "if(i_err/=0) write(*,*) 'ERROR: Reading ',trim(keyword),' data',i_err"},
...
FortranOutputs[] := Module[{Fint, Fout},
  ...
  Fout = {{"Module Outputs "},
          ...
          {Fint[2] <> "Real*8          :: Etot, e11, e22, e33, e23, e13, e12 "},
...
Makefile[dir_] := Module[{Fint, Fout},
  ...
  Fout = {{".SUFFIXES: .f .f90 .F90 .SUFFIXES .prj"},
          ...
          {"SRCS = Parameters.f90 Constants.f90 FileParser.f90 \\"},
          {"       Inputs.f90 \\"},
          {"       LINVARIANT.f90 \\"},
          {"       EwaldMatrix.f90 \\"},
          {"       Outputs.f90 \\"},
