...
FortranLatticeTikTok[OptionsPattern[{"AllSites"->True}]] := Module[{Fint, n, Fout},
  ...
  Fout = Join[
       {{"Subroutine TikTok(Fields, dFieldsdt, e0ij, T0, Tk, gm)"},
        ...
        {Fint[1] <> "Real*8,  Intent(inout) :: Tk, gm"},
        ...
        {Fint[1] <> "Real*8                 :: dFields0dt(FieldDim, NumField, NGridx, NGridy, NGridz)"}},
       If[OptionValue["AllSites"], {{Fint[1] <> "Real*8                 :: Forces(FieldDim,NumField,NGridx,NGridy,NGridz)"}}, 
                                   {{Fint[1] <> "Real*8                 :: Forces(FieldDim,NumField)"}}],
       {{Fint[1] <> "Integer                :: i, ifield, fi, ix, iy, iz"},
        {Fint[1]},
        {Fint[1] <> "Fields0 = Fields"},
        {Fint[1] <> "dFields0dt = dFieldsdt"},
        {Fint[1]}},
       If[OptionValue["AllSites"], 
         {{Fint[1] <> "If(DipoleQ) then"},
          {Fint[2] <> "Forces = GetEwaldForces(Fields0) + GetForces(Fields0, e0ij)"},
          {Fint[1] <> "else"},
          {Fint[2] <> "Forces = GetForces(Fields0, e0ij)"},
          {Fint[1] <> "end if"},
          {Fint[1]},
          ...
          {Fint[2] <> "dFieldsdt(i, ifield, ix, iy, iz) = dFields0dt(i, ifield, ix, iy, iz) &"},
          {Fint[3] <> "+ 0.5d0*DeltaT/7.464D0*(Forces(i, ifield, ix, iy, iz)/mass-gm*dFields0dt(i, ifield, ix, iy, iz))"},
          {Fint[2] <> "Fields(i, ifield, ix, iy, iz) = &"},
          {Fint[3] <> "Fields0(i, ifield, ix, iy, iz) + dFieldsdt(i, ifield, ix, iy, iz)*DeltaT/7.464D0"},
          {Fint[1] <> "end do"},
          {Fint[1] <> "end do"},
          {Fint[1] <> "end do"},
          {Fint[1] <> "end do"},
         ...
         {{Fint[1] <> "If(DipoleQ) then"},
          ...
          {Fint[3] <> "Forces = GetEwaldForces(ix,iy,iz,Fields0) + GetForces(ix,iy,iz,Fields0,e0ij)"},
          {Fint[3] <> "Forces(:,1) = Forces(:,1)/mass(1)"},
          {Fint[3] <> "Forces(:,2) = Forces(:,1)/mass(2)"},
          {Fint[3] <> "Forces(:,3) = Forces(:,1)/mass(3)"},
          {Fint[3] <> "dFieldsdt(:,:,ix,iy,iz) = dFields0dt(:,:,ix,iy,iz) &"},
          {Fint[4] <> "+ 0.5d0*DeltaT/7.464D0*(Forces(:,:)-gm*dFields0dt(:,:,ix,iy,iz))"},
          {Fint[3] <> "Fields(:,:,ix,iy,iz) = Fields0(:,:,ix,iy,iz) + dFieldsdt(:,:,ix,iy,iz)*DeltaT/7.464D0"},
          {Fint[2] <> "end do"},
          {Fint[2] <> "end do"},
          {Fint[2] <> "end do"},
          {Fint[1] <> "!$OMP    END DO"},
          ...
          {Fint[2] <> "do iy = 1, NGridy"},
          {Fint[2] <> "do ix = 1, NGridx"},
          {Fint[3] <> "Forces = GetForces(ix,iy,iz,Fields0,e0ij)"},
          {Fint[3] <> "Forces(:,1) = Forces(:,1)/mass(1)"},
          {Fint[3] <> "Forces(:,2) = Forces(:,1)/mass(2)"},
          {Fint[3] <> "Forces(:,3) = Forces(:,1)/mass(3)"},
          {Fint[3] <> "dFieldsdt(:,:,ix,iy,iz) = dFields0dt(:,:,ix,iy,iz) &"},
          ...
          {Fint[1] <> "end if"},
          {Fint[1]}}],
       {{Fint[1] <> "gm = gm + 0.5D0*NGridx*NGridy*NGridz*OnSiteDim*(Tk-T0)*0.5D0*DeltaT/7.464D0/NoseMass"},
        {Fint[1] <> "Tk = Thermometer(dFieldsdt, e0ij)"},
        {Fint[1] <> "gm = gm + 0.5D0*NGridx*NGridy*NGridz*OnSiteDim*(Tk-T0)*0.5D0*DeltaT/7.464D0/NoseMass"},
        {Fint[1]}},
       If[OptionValue["AllSites"], 
         {{Fint[1] <> "If(DipoleQ) then"},
          ...
          {Fint[1] <> "do ifield = 1, NumField"},
          {Fint[1] <> "do i = 1, 3"},
          {Fint[2] <> "dFieldsdt(i, ifield, ix, iy, iz) = &"},
          {Fint[3] <> "(dFieldsdt(i, ifield, ix, iy, iz) &"},
          {Fint[3] <> "+ 0.5d0*DeltaT/7.464D0*Forces(i, ifield, ix, iy, iz)/mass)/(1.0D0 + 0.5D0*gm*DeltaT/7.464D0)"},
          {Fint[1] <> "end do"},
          {Fint[1] <> "end do"},
          {Fint[1] <> "end do"},
          {Fint[1] <> "end do"},
         ...
         {{Fint[1] <> "If(DipoleQ) then"},
          ...
          {Fint[2] <> "do iy = 1, NGridy"},
          {Fint[2] <> "do ix = 1, NGridx"},
          {Fint[3] <> "Forces = GetEwaldForces(ix,iy,iz,Fields) + GetForces(ix,iy,iz,Fields,e0ij)"},
          {Fint[3] <> "Forces(:,1) = Forces(:,1)/mass(1)"},
          {Fint[3] <> "Forces(:,2) = Forces(:,1)/mass(2)"},
          {Fint[3] <> "Forces(:,3) = Forces(:,1)/mass(3)"},
          {Fint[3] <> "dFieldsdt(:,:,ix,iy,iz) = (dFieldsdt(:,:,ix,iy,iz) &"},
          {Fint[4] <> "+ 0.5d0*DeltaT/7.464D0*Forces(:,:))/(1.0D0 + 0.5D0*gm*DeltaT/7.464D0)"},
          {Fint[2] <> "end do"},
          {Fint[2] <> "end do"},
          {Fint[2] <> "end do"},
          {Fint[1]},
...
FortranLatticeMCStep[OptionsPattern[{"EwaldField"->True, "SwapAll"->False}]] := Module[{Fint, n, Fout, EwaldFieldArg, EwaldFieldBlock},
  ...
  Fout = Join[{
    ...
    {Fint[1] <> "Integer, Intent(inout) :: imc, iseed"},
   ...
   {{Fint[1] <> "Real*8                 :: ProbTest"},
    ...
    {Fint[5] <> "end if"},
    {Fint[1]},
    {Fint[5] <> "AccProb = Min(1.0d0, Exp(-1.0d0*DeltaE/T))"},
  ...
  {{Fint[5] <> "end if"},
    ...
    {Fint[2]},
    {Fint[2] <> "AccProb = Min(1.0d0, Exp(-1.0d0*DeltaE/T))"},
...
FortranEOnSiteEwald[OptionsPattern[{}]] := Module[{Fint, n, Fout},
  ...
  Fout = {{"Function GetEOnSiteEwald(ix, iy, iz, Fields) Result(ene)"},
          ...
          {Fint[3] <> "do jfield = 1, NumField"},
          {Fint[3] <> "do ifield = 1, NumField"},
          {Fint[3] <> "do j = 1, FieldDimList(jfield)"},
          ...
          {Fint[1] <> "!$OMP    END PARALLEL"},
          {Fint[1]},
          {Fint[1] <> "do jfield = 1, NumField"},
          {Fint[2] <> "do ifield = 1, NumField"},
          {Fint[3] <> "do j = 1, 3"},
          {Fint[4] <> "do i = 1, 3"},
          {Fint[5] <> "ene = ene &"},
...
FortranGetEwaldForces[OptionsPattern[{"AllSites" -> True}]] := Module[{Fint, n, Fout, func},
  ...
  Fout = Join[
    ...
    {{Fint[1] <> "use Parameters"},
     ...
     {Fint[1] <> "real*8                        :: EwaldForce" <> If[OptionValue["AllSites"], "(FieldDim, NumField, NGridx, NGridy, NGridz)", "(FieldDim, NumField)"]}, 
     {Fint[1] <> "real*8                        :: force"},
     {Fint[1]},
...
HeadTailDeltaH[FunctionName_?StringQ, ReturnVar_, nn_, OptionsPattern[{}]] := Module[{Fint, n, head, tail, res, reslist},
  ...
  head = Join[{{"Function " <> FunctionName <> "(x0, y0, z0, Fields, e0ij, delta, deta, idelta) Result(" <> res <> ")"},
               {Fint[1]},
               {Fint[1] <> "Implicit none"},
               {Fint[1] <> "Integer, Intent(in) :: x0, y0, z0, idelta"},
               {Fint[1] <> "Real*8,  Intent(in) :: Fields(FieldDim, NumField, NGridx, NGridy, NGridz)"}},
              {{Fint[1] <> "Real*8,  Intent(in) :: delta(FieldDim, NumField)"},
               {Fint[1] <> "Real*8,  Intent(in) :: deta(6)"}}, 
 ...
 Module[{Fint, n, head, tail, res, reslist, func},
  ...
  head = Join[ 
      ...
      {{Fint[1]},
       ...
       {Fint[1] <> "Real*8              :: eij(3,3), euij(3,3)"}},
       {Fint[1] <> "Real*8              :: " <> # <> If[OptionValue["AllSites"], "(FieldDim, NumField, NGridx, NGridy, NGridz)", "(FieldDim, NumField)"]} & /@ DeleteDuplicates[Join[{res}, reslist]],
      {{Fint[1]}},
      HopingCodeBlock[nn][[2]],
      {{Fint[1]}},
      If[OptionValue["AllSites"], 
