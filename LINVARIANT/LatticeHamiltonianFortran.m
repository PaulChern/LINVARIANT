...
FortranLatticePTSwap[] := Module[{Fint, Fout},
  ...
  Fout = {{"Subroutine GetPTSwapMap(TempList, EtotList, NumReplicas, Replicas)"},
          ...
          {Fint[1]},
          {Fint[1]},
          {"Subroutine PTSwap(TempList, Fields, EwaldField, e0ij, dFieldsdt, de0ijdt, NumReplicas, Replicas, ireplica, istep)"},
          {Fint[1]},
          {Fint[1] <> "Use LINVARIANT"},
          {Fint[1] <> "Use Constants"},
          {Fint[1] <> "Use Inputs"},
          ...
          {Fint[2] <> "MPI_DETADT_Recv,6,MPI_DOUBLE_PRECISION,&"},
          {Fint[2] <> "MPI_COMM_WORLD,IERROR)"},
          {Fint[2] <> "call MPI_ALLGATHER(MPI_DFIELDDT_1D,DimFields1D,MPI_DOUBLE_PRECISION,&"},
          {Fint[2] <> "MPI_DFIELDDT_1D_Recv,DimFields1D,MPI_DOUBLE_PRECISION,&"},
          {Fint[2] <> "MPI_COMM_WORLD,IERROR)"},
          {Fint[1] <> "end if"},
          {Fint[1]},
