Subroutine MCMCStep(imc, Fields, EwaldField, e0ij, T, udamp, etadamp, dene)
    ...
    do iy = 1, cgrid%n2
      do ix = 1, cgrid%n1
        do idelta = 1, NumField
          ...
          DeltaE = GetDeltaH(ix, iy, iz, Fields, e0ij, idelta, dfield) + GetDeltaHJijsmhf(ix, iy, iz, Fields, e0ij, idelta, dfield)
  ...
  do idum = 1, 2*cgrid%n3+1
    ...
    DeltaE = DeltaE + cgrid%npts*GetDeltaHEps(1, 1, 1, Fields, e0ij, NumField+1, deta)
    ...
    do iy = 1, cgrid%n2
      do ix = 1, cgrid%n1
        do idelta = 1, NumField
          ...
          dfield = udamp(idelta)*(dfield - 0.5D0)
          do i = 1, FieldDimList(idelta)
            Fields_tmp(i,idelta,ix,iy,iz) = Fields(i,idelta,ix,iy,iz) + dfield(i)
          end do
