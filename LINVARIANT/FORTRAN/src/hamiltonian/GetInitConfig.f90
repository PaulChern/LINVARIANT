    ...
    CASE ("random")
      do iz = 1, cgrid%n3
        do iy = 1, cgrid%n2
          do ix = 1, cgrid%n1
            do i = 1, NumField
              do j = 1, FieldDimList(i)
                Call random_number(rand)
                Fields(j,i,ix,iy,iz) = (rand-0.5d0)/1.0d1
              end do
            end do
          end do
        end do
    ...
    CASE ("phase")
      ...
      if (file_exists .eqv. .False.) then
        ...
        do ifield = 1, NumField
          Read(FileHandle, "(3E25.15)") (field(i, ifield), i=1,FieldDimList(ifield))
          do i = 1, FieldDimList(ifield)
            do iz = 1, cgrid%n3
              do iy = 1, cgrid%n2
                do ix =1, cgrid%n1
                  qdotr=Real(Exp(2*pi*cmplx(0.0_dp,1.0_dp)*(DWq(1,i)*ix+DWq(2,i)*iy+DWq(3,i)*iz)))
                  Fields(i,ifield,ix,iy,iz) = field(i,ifield)*tanh(100.0*qdotr)
                end do
              end do
            end do
          end do
