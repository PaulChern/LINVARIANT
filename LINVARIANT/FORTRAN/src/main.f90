...
Program main
  ...
  Real*4                   :: tstart, tend
  Real*8                   :: e0ij(3,3), de0ijdt(3,3), Tk(2), dene
...
!    Call SemiCircleContour
  ...
  Call GetEwaldHessian(EwaldMat, EwaldHessian)
  ...
  write(*,*) "Ewald field"
  allocate(EwaldField(FieldDim,NumField,cgrid%n1,cgrid%n2,cgrid%n3))
  ...
  If(trim(Solver).eq."Minimization") then
    ...
    do istep = 1, NumSteps
      do i = 1, NumField
        If(istep.gt.ThermoSteps.and.CLAMPQ(i)) dFieldsdt(:,i,:,:,:) = 0.0D0
      end do
      If(CLAMPQ(NumField+1)) de0ijdt = 0.0D0
      ...
      do i_wl = 1, wl_NumSteps
        ...
        if(istep.gt.ThermoSteps) then
          ...
          do i = 1, ContourNPoints(2)
            ...
            write(ifileno, "(3E25.15)") ((Sum(Fields(j,ifield,:,:,:))/cgrid%npts,j=1,FieldDimList(ifield)),ifield=1,NumField)
          end do 
        end if
      end do
      wl_f = wl_f**wl_a
