Module Aux
  ...
  Function FieldTo1D(Fields, ifield) Result(Field1D)
    ...
    Real*8                 :: Field1D(FieldDimList(ifield)*NumField*cgrid%npts)
      ...
      do iy = 1, cgrid%n2
        do ix = 1, cgrid%n1
          do i = 1, FieldDimList(ifield)
            id = (iz-1)*cgrid%n2*cgrid%n1*FieldDimList(ifield) &
            + (iy-1)*cgrid%n1*FieldDimList(ifield) &
            + (ix-1)*FieldDimList(ifield) &
            + i
  ...
  Subroutine SysVector2Fields(sysvector, Fields, e0ij)
    ...
    Fields = Reshape(sysvector(1:FieldDim*NumField*cgrid%npts), &
