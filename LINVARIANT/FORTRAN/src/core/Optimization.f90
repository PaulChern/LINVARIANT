  ...
  subroutine OptEnergyFunc(val, n, x, grad, need_gradient)
    ...
    if (DipoleQ) then
      do iz = 1, cgrid%n3
        do iy = 1, cgrid%n2
          do ix = 1, cgrid%n1
            val = val + GetEOnSite(ix, iy, iz, Fields, e0ij) + GetEOnSiteEwald(ix, iy, iz, Fields) + GetEOnSiteJijsmhf(ix,iy,iz,Fields,e0ij)
          end do
        end do
      end do
    else
      do iz = 1, cgrid%n3
        do iy = 1, cgrid%n2
          do ix = 1, cgrid%n1
            val = val + GetEOnSite(ix, iy, iz, Fields, e0ij) + GetEOnSiteJijsmhf(ix,iy,iz,Fields,e0ij)
          end do
        end do
      end do
    end if
